...
class Short_Leave_Entry extends CI_Controller
{
    ...
    public function insert_data()
    {
        ...
        if (!empty($useentity["sh"])) {
            ...
            $dateString = $useentity["sh"][0]->Date;
    ...
    public function insert_data1()
    {
        ...
        $ApproveUser = $currentUser[0]->EmpNo;
        $Emp = $this->input->post('txt_emp');
        $date1 = $this->input->post('att_date'); // Format: yyyy/mm/dd
        $from_time = $this->input->post('in_time');
        $to_time = $this->input->post('out_time');

        date_default_timezone_set('Asia/Colombo');
        $currentDate = new DateTime();
        $todayYear = $currentDate->format('Y');
        $dateTime = $currentDate->format('Y/m/d h:i:s');

        // Extract year and month from posted date
        $orderdate1 = explode('/', $date1);
        $inputYear = $orderdate1[0] ?? '';
        $inputMonth = $orderdate1[1] ?? '';

        $useentity["sh"] = $this->Db_model->getfilteredData("SELECT * FROM tbl_shortlive WHERE `EmpNo` = '$Emp' AND `Month`='$inputMonth'");

        if (!empty($useentity["sh"])) {
            foreach ($useentity["sh"] as $data) {
                if ($data != null) {
                    $existingDate = new DateTime($data->Date);
                    $existingYear = $existingDate->format('Y');

                    if ($existingYear == $inputYear) {
                        $this->session->set_flashdata('error_message', 'Already Have a Short Leave');
                        redirect('Leave_Transaction/Short_Leave_Entry/index');
                        return;
                    }
        ...
        // Proceed to insert if not duplicate for the same year
        $insertData = array(
            'EmpNo' => $Emp,
            'from_time' => $from_time,
            'to_time' => $to_time,
        ...
        $this->Db_model->insertData('tbl_shortlive', $insertData);
        $this->session->set_flashdata('success_message', 'Employee Short Leave Added');
        redirect('Leave_Transaction/Short_Leave_Entry/index');
    }
