...
<script>
    document.getElementById("submit").addEventListener("click", function (e) {
        ...
        // 1. Get group name
        const groupName = document.getElementById("txt_group_name").value;

        // 2. Get supervisor list from the table
        const supervisors = [];
        const rows = document.querySelectorAll("#sortableRows tr");

        rows.forEach(row => {
            const supervisorId = row.getAttribute("data-id");
            const supervisorName = row.cells[1].textContent;
        ...
        // 4. Use the model (e.g., log it or send to server)
        console.log("Collected Data Model:", dataModel);

        // ✅ Send via AJAX if needed:
        /*
        $.ajax({
