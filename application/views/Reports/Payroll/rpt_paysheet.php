...
function generateDepartmentPDF($pdf, $ass_department_name, $ass_department_data, $data_year, $data_month) {
    ...
    <table cellpadding="6" style="margin-left: -10px;">
        <thead style="border-bottom: #000 solid 1px;">
            <tr style="border-bottom: 1px solid black; font-weight:bold"> 
                ...
                <th style="width:70px; font-size: 7px;">PERFORMANCE</th>
                <th style="width:55px;">ATTENDANCES</th>
                <th style="width:40px;">TRANSPORT</th>
                ...
                <th style="width:50px;">OTHER OT</th>
                <th style="width:55px;">GROSS PAY</th>

                <th style="width:50px; font-size: 7px;">ADV.PAID</th>
                <th style="width:60px;">P.A.Y.E</th>
                <th style="width:55px;">MOBILE-DED</th>
                <th style="width:55px;">FUEL-DED</th>
                <th style="width:55px;">WELFAIR</th>
                <th style="width:55px;">NO PAY</th>              
    ...
    foreach ($ass_department_data as $data) {
        ...
        <tr>
            ...
            <td style="width:100px;">' . $data->Dep_Name . '</td>                                           
            ...
            <td style="width:70px;">' . number_format($data->Performance, 2, '.', ',') . '</td>
            <td style="width:55px;">' . number_format($data->Attendances, 2, '.', ',') . '</td>
            <td style="width:40px;">' . number_format($data->Transport, 2, '.', ',') . '</td>
            ...
            <td style="width:50px;">' . number_format($data->Other_OT, 2, '.', ',') . '</td>
            <td style="width:55px;">' .  number_format($data->Gross_pay, 2, '.', ',') . '</td>

            <td style="width:50px;">' . number_format($data->Salary_advance, 2, '.', ',')  . '</td>
            <td style="width:60px;">' . number_format($data->Payee_amount, 2, '.', ',') . '</td>
            <td style="width:55px;">' . number_format($data->Mobile_Ded, 2, '.', ',') . '</td>
            <td style="width:55px; ">' . number_format($data->Fuel_Ded, 2, '.', ',') . '</td>                                                              
            <td style="width:55px; ">' . number_format($data->Wellfare, 2, '.', ',') . '</td>                                                              
            <td style="width:55px;">' . number_format($data->no_pay_deduction, 2, '.', ',') . '</td>
